...
use crate::{
    ...
    db::{
        ...
        models::{
            ...
            MembershipStatus, OrgPolicy, OrgPolicyType, Organization, OrganizationId, Send, SendId, User, UserId, UserKdfType,
        },
    },
    mail,
...
async fn get_key_connector_user_keys(headers: Headers) -> JsonResult {
    ...
    if let Some(ref url) = CONFIG.sso_key_connector_url() {
        let user_id = headers.user.uuid.as_ref();
        let resp = reqwest::Client::new()
            .get(format!("{url}/user-keys?user_id={user_id}"))
            .send().await
            .map_err(|e| crate::error::Error::new("Key Connector request failed", e.to_string()))?;
        if resp.status().is_success() {
            let body: Value = resp.json().await?;
            return Ok(Json(body));
        }
        err!("Key not found on external Key Connector");
    }
...
async fn post_key_connector_user_keys(data: Json<KeyConnectorKeyData>, headers: Headers) -> EmptyResult {
    ...
    if let Some(ref url) = CONFIG.sso_key_connector_url() {
        let user_id = headers.user.uuid.as_ref();
        let resp = reqwest::Client::new()
            .post(format!("{url}/user-keys?user_id={user_id}"))
            .json(&json!({ "key": data.key }))
            .send().await
            .map_err(|e| crate::error::Error::new("Key Connector request failed", e.to_string()))?;
        if !resp.status().is_success() {
            err!("Failed to store key on external Key Connector");
        }
        info!("Stored Key Connector key for user {} (external)", headers.user.email);
        return Ok(());
    }
    ...
    match org {
        Some(org) => Ok(Json(json!({
            ...
            "keyConnectorUrl": CONFIG.sso_key_connector_url()
                .unwrap_or_else(|| format!("{}/api/key-connector", CONFIG.domain())),
        }))),
