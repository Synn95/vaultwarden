...
make_config! {
    ...
    sso {
        ...
        /// SSO Key Connector secret |> Required when SSO_KEY_CONNECTOR=true and SSO_KEY_CONNECTOR_URL is not set. 64-char hex string (256 bits) used to encrypt user keys at rest via AES-256-GCM. Generate with: openssl rand -hex 32. CRITICAL: back up this secret — losing it means all Key Connector users lose vault access.
        sso_key_connector_secret:       Pass,   true,   option;
        /// SSO Key Connector URL |> URL of an external Key Connector service. When set, Vaultwarden delegates key storage to this service instead of using the built-in encrypted file storage. The external service must implement GET/POST /user-keys with Bearer token auth. This enables true trust-boundary separation — the vault server never handles master keys.
        sso_key_connector_url:          String, true,   option;
...
fn validate_config(cfg: &ConfigItems, on_update: bool) -> Result<(), Error> {
    ...
    if cfg.sso_enabled {
        ...
        if cfg.sso_key_connector {
            let has_external_url = cfg.sso_key_connector_url.is_some();
            if !has_external_url {
                match &cfg.sso_key_connector_secret {
                    None => err!("`SSO_KEY_CONNECTOR_SECRET` is required when using built-in Key Connector (no `SSO_KEY_CONNECTOR_URL`). Generate with: openssl rand -hex 32"),
